...
$repo = (new WooCommerceConnectorService())->getRepository();
...
<div class="dashboard-grid-main-aside">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><?= htmlspecialchars((string)($row['store_url'] ?? '-')) ?></td>
  ...
  <div class="card">
    ...
    <p style="color:var(--text-mid);line-height:1.7;margin-bottom:12px">
      Tahap 2 WooCommerce connector sudah menyiapkan live catalog pull, live order push, dan inbound webhook status. Pantau hasil percobaan tiap cabang dari log di bawah ini.
    </p>
    <div style="display:flex;flex-direction:column;gap:10px;max-height:720px;overflow:auto">
      ...
      <?php endforeach; ?>
      <?php if ($logs === []): ?>
      <div style="color:var(--text-light)">Belum ada activity log WooCommerce.</div>
      <?php endif; ?>
    </div>
    <a href="<?= BASE_URL ?>/dashboard/super/settings.php" class="btn btn-outline btn-sm" style="margin-top:14px">Kelola Pengaturan WooCommerce</a>
  </div>
</div>
<?php
$content = ob_get_clean();
echo View::renderLayout('WooCommerce Connector', $content, 'super_admin');
