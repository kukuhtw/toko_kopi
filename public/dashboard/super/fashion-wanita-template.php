...
$plugin = PluginLoader::get('fashion-wanita-template');
$result = null;

if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'seed') {
    $result = $plugin && method_exists($plugin, 'resetAndSeed')
        ? $plugin->resetAndSeed()
        : ['success' => false, 'message' => 'Plugin fashion-wanita-template tidak aktif.'];
}
...
ob_start(); ?>
<div class="card">
    <h2 style="margin-bottom:12px">👗 Toko Baju Busana Wanita Template</h2>
    <p style="margin-bottom:12px">Plugin template untuk toko baju, butik, dan fashion wanita online. Menyediakan <strong>80 produk</strong> dalam 8 kategori.</p>
    ...
    <?php if ($plugin): ?>
        <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:14px;margin-bottom:16px;font-size:.875rem">
            <strong>⚠️ Perhatian:</strong> Reset & Seed akan menghapus <strong>semua data produk, kategori, dan pesanan</strong>, lalu mengisi 80 produk fashion wanita. Tidak dapat dibatalkan.
        </div>
        <form method="POST" onsubmit="return confirm('Yakin reset dan seed 80 produk fashion wanita?')">
            <input type="hidden" name="action" value="seed">
            <button type="submit" class="btn btn-primary">👗 Reset & Seed Fashion Wanita (80 produk)</button>
        </form>
    <?php else: ?>
        <div class="alert alert-warning">Plugin <code>fashion-wanita-template</code> belum aktif. Aktifkan di <a href="<?= BASE_URL ?>/dashboard/super/plugins.php">Plugins</a>.</div>
    <?php endif; ?>
</div>
<?php
$content = ob_get_clean();
echo View::renderLayout('Fashion Wanita Template', $content, 'super_admin');
