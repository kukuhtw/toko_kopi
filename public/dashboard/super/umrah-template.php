...
$plugin = PluginLoader::get('umrah-template');
$result = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'seed') {
    if ($plugin && method_exists($plugin, 'resetAndSeed')) {
        ...
        $result = ['success' => false, 'message' => 'Plugin umrah-template tidak aktif atau method resetAndSeed tidak ditemukan.'];
    }
}
...
?>
<div class="card">
    <h2 style="margin-bottom:12px">🕋 Umrah Template</h2>

    <p style="margin-bottom:12px">
        Template untuk travel umrah, paket ibadah, handling dokumen, dan perlengkapan jamaah.
        Menyediakan <strong>15 layanan</strong> dalam 3 kategori.
    </p>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px">
        <div style="background:#f8f9fa;border-radius:8px;padding:16px">
            ...
            <div style="font-size:.85rem;color:#666">Paket &amp; Layanan</div>
        </div>
        <div style="background:#f8f9fa;border-radius:8px;padding:16px">
    ...
    <ul style="line-height:1.9;margin-bottom:20px;font-size:.9rem">
        <li>Umrah Reguler 9 Hari, 12 Hari, Ramadhan, Akhir Tahun, Plus Thaif</li>
        <li>Umrah VIP, Plus Turki, Plus Dubai, Family Private, Business Class</li>
        <li>Paspor &amp; Dokumen, Vaksin, Perlengkapan, Manasik Private, Badal Umrah</li>
    </ul>
    ...
    <?php if ($plugin): ?>
        <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:14px;margin-bottom:16px;font-size:.875rem">
            ...
            lalu menggantinya dengan 15 paket dan layanan umrah.
        </div>
        <form method="POST" onsubmit="return confirm('Yakin reset dan seed 15 paket & layanan umrah? Semua data produk dan pesanan akan dihapus.')">
            <input type="hidden" name="action" value="seed">
            <button type="submit" class="btn btn-primary">
                🕋 Reset &amp; Seed Umrah (15 layanan)
            </button>
        </form>
    <?php else: ?>
        <div class="alert alert-warning">
            Plugin <code>umrah-template</code> belum aktif. Aktifkan melalui halaman
...
echo View::renderLayout('Umrah Template', $content, 'super_admin');
