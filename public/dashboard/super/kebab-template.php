...
$plugin = PluginLoader::get('kebab-template');
$result = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'seed') {
    if ($plugin && method_exists($plugin, 'resetAndSeed')) {
        ...
        $result = ['success' => false, 'message' => 'Plugin kebab-template tidak aktif atau method resetAndSeed tidak ditemukan.'];
    }
}
...
?>
<div class="card">
    <h2 style="margin-bottom:12px">🌯 Kebab Template</h2>

    <p style="margin-bottom:12px">
        Plugin template untuk kedai kebab, shawarma, dan street food Timur Tengah.
    ...
    <ul style="line-height:1.9;margin-bottom:20px;font-size:.9rem">
        <li>Kebab Original, Kebab Ayam Crispy, Kebab Mozarella</li>
        <li>Kebab Double Beef, Kebab Spesial Pedas, Kebab Smoked Beef</li>
        <li>Kebab Veggie, Shawarma Ayam, Pita Wrap Falafel</li>
        <li>Kebab Box Komplit (paket)</li>
        <li>Es Teh Manis, Es Jeruk, Milkshake Coklat, Milkshake Vanilla, Air Mineral</li>
    </ul>
    ...
    <?php if ($plugin): ?>
        <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:14px;margin-bottom:16px;font-size:.875rem">
            ...
            lalu mengisi ulang dengan 15 menu kebab. Proses ini tidak dapat dibatalkan.
        </div>
        <form method="POST" onsubmit="return confirm('Yakin reset dan seed 15 menu kebab? Semua data produk dan pesanan akan dihapus.')">
            <input type="hidden" name="action" value="seed">
            <button type="submit" class="btn btn-primary">
                🌯 Reset & Seed Menu Kebab (15 menu)
            </button>
        </form>
    <?php else: ?>
        <div class="alert alert-warning">
            Plugin <code>kebab-template</code> belum aktif. Aktifkan melalui halaman
...
echo View::renderLayout('Kebab Template', $content, 'super_admin');
