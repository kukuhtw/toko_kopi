...
/** @var CoffeeTemplatePlugin|null $plugin */
$plugin = PluginLoader::get('coffee-template');
if (!$plugin) {
    echo View::renderLayout(
        'Coffee Shop Template',
        '<div class="card"><p style="color:var(--accent-red)">Plugin "coffee-template" tidak aktif. Aktifkan di halaman Plugins.</p></div>',
        'super_admin'
    );
    exit;
}
...
$categories    = CoffeeTemplatePlugin::getCategories();
$menuItems     = CoffeeTemplatePlugin::getMenuItems();
$totalItems    = 0;
$totalVariants = 0;
...
<style>
.coffee-warning-box {
  background: rgba(101,67,33,.07);
  border: 2px solid rgba(101,67,33,.3);
  border-radius: var(--radius-lg);
  padding: 20px 24px;
  margin-bottom: 24px;
}
.coffee-warning-box h3 {
  margin: 0 0 10px; color: #6b3b1a;
  font-size: 1rem; display: flex; align-items: center; gap: 8px;
}
.coffee-warning-box ul {
  margin: 0 0 0 18px; padding: 0;
  color: #5c3317; font-size: .88rem; line-height: 1.7;
}
.coffee-confirm-row {
...
.coffee-confirm-row input[type=checkbox] {
  width: 18px; height: 18px; accent-color: #c2410c;
  flex-shrink: 0; cursor: pointer;
}
.coffee-confirm-row span { font-size: .9rem; font-weight: 600; color: #7c2d12; }

.coffee-seed-btn {
  margin-top: 20px; padding: 12px 28px;
  background: #6b3b1a; color: #fff; border: none;
  border-radius: 8px; font-size: .95rem; font-weight: 700;
  cursor: pointer; transition: background .15s, opacity .15s;
}
.coffee-seed-btn:disabled { opacity: .4; cursor: not-allowed; }
.coffee-seed-btn:not(:disabled):hover { background: #5c3317; }
...
.coffee-cat-card h4 { margin: 0 0 8px; font-size: .9rem; color: var(--text-dark); }
.coffee-item-list {
  list-style: none; margin: 0; padding: 0;
  font-size: .8rem; color: var(--text-mid); line-height: 1.8;
}
.coffee-item-list li::before { content: '• '; color: #6b3b1a; }

.coffee-stat-row { display: flex; gap: 16px; flex-wrap: wrap; margin-bottom: 20px; }
.coffee-stat {
  background: var(--coffee-white, #fdfaf7);
  border: 1px solid var(--border); border-radius: 10px;
  padding: 14px 20px; text-align: center; min-width: 110px;
}
.coffee-stat .stat-num { font-size: 1.8rem; font-weight: 800; color: #6b3b1a; }
.coffee-stat .stat-lbl { font-size: .75rem; color: var(--text-mid); margin-top: 2px; }
...
<div class="card" style="margin-bottom:24px">
  <h2 style="margin:0 0 16px;font-size:1.05rem">Ringkasan Template Toko Kopi</h2>
  <div class="coffee-stat-row">
  ...
  <p style="margin:8px 0 0;font-size:.85rem;color:var(--text-mid)">
    Data menu berasal dari <strong>database asli toko kopi</strong> ini.
...
<div class="card">
  ...
  <div class="coffee-warning-box">
    ...
    <ul>
      ...
      <li>Data akan diganti dengan <?= $totalItems ?> menu toko kopi beserta <?= $totalVariants ?> variannya.</li>
    ...
    <label class="coffee-confirm-row" id="confirmLabel">
  ...
  <form method="POST">
    ...
    <button type="submit" class="coffee-seed-btn" id="seedBtn" disabled>
      Jalankan Reset &amp; Seed Template Toko Kopi
    </button>
  </form>
</div>
...
echo View::renderLayout('Coffee Shop Template', $content, 'super_admin');
