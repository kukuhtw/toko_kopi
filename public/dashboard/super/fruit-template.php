...
/** @var FruitTemplatePlugin|null $plugin */
$plugin = PluginLoader::get('fruit-template');
if (!$plugin) {
    echo View::renderLayout(
        'Fruit Template',
        '<div class="card"><p style="color:var(--accent-red)">Plugin "fruit-template" tidak aktif. Aktifkan di halaman Plugins.</p></div>',
        'super_admin'
    );
    exit;
}
...
$categories    = FruitTemplatePlugin::getCategories();
$menuItems     = FruitTemplatePlugin::getMenuItems();
$totalItems    = 0;
$totalVariants = 0;
...
.fruit-confirm-row input[type=checkbox] {
  width: 18px; height: 18px;
  accent-color: #c2410c;
  flex-shrink: 0; cursor: pointer;
}
.fruit-confirm-row span {
  font-size: .9rem; font-weight: 600; color: #7c2d12;
}
.fruit-seed-btn {
  margin-top: 20px;
  padding: 12px 28px;
...
.fruit-seed-btn:disabled { opacity: .4; cursor: not-allowed; }
.fruit-seed-btn:not(:disabled):hover { background: #15803d; }
...
.fruit-cat-card h4 { margin: 0 0 8px; font-size: .9rem; color: var(--text-dark); }
.fruit-item-list {
  list-style: none; margin: 0; padding: 0;
  font-size: .8rem; color: var(--text-mid); line-height: 1.8;
}
.fruit-item-list li::before { content: '• '; color: #16a34a; }

.fruit-stat-row { display: flex; gap: 16px; flex-wrap: wrap; margin-bottom: 20px; }
.fruit-stat {
  background: var(--coffee-white, #fdfaf7);
...
.fruit-stat .stat-num { font-size: 1.8rem; font-weight: 800; color: #16a34a; }
.fruit-stat .stat-lbl { font-size: .75rem; color: var(--text-mid); margin-top: 2px; }
...
<div class="card" style="margin-bottom:24px">
  <h2 style="margin:0 0 16px;font-size:1.05rem">Ringkasan Template Toko Buah</h2>
  <div class="fruit-stat-row">
    <div class="fruit-stat">
...
<div class="card">
  ...
  <div class="fruit-warning-box">
    ...
    <ul>
      ...
      <li>Data akan diganti dengan <?= $totalItems ?> menu toko buah baru beserta <?= $totalVariants ?> variannya.</li>
    ...
    <label class="fruit-confirm-row" id="confirmLabel">
  ...
  <form method="POST">
    ...
    <button type="submit" class="fruit-seed-btn" id="seedBtn" disabled>
      Jalankan Reset &amp; Seed Template Toko Buah
    </button>
  </form>
</div>
...
echo View::renderLayout('Fruit Store Template', $content, 'super_admin');
