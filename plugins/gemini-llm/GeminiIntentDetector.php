...
- tambah_item: ordering or ADDING NEW items to cart
- ubah_item: changing quantity of a cart item
- hapus_item: removing a specific item from cart
- clear_cart: clearing the entire cart
- lihat_cart: viewing current cart contents
...
- konfirmasi_order: confirming the order
- tanya_status_order: asking about past orders or order history
- batal_order: cancelling order
- small_talk: greeting, thank you, casual chat
- out_of_scope: anything unrelated to the {$businessType}

IMPORTANT:
- "tadi saya pesan apa?", "riwayat order", "history pesanan", "pernah order disini?" = tanya_status_order
- "saya pesan apa?", "pesan apa saya?", "keranjang saya" = lihat_cart
- "[item] jadi [number]" = ubah_item
- Catalog explanation/recommend requests = tanya_menu
...
- item_query: menu item name in lowercase
...
SYS;
    ...
    public function detectAll(string $message, array $context = []): array
    {
        ...
        if (in_array($state, ['awaiting_name','awaiting_email','awaiting_wa','awaiting_address',
            'awaiting_postal','awaiting_confirmation'], true)) {
        ...
        try {
            ...
            error_log('[GeminiIntentDetector] detectAll: ' . $e->getMessage());
        }
    ...
    {
        $state = $context['state'] ?? 'idle';
        if (in_array($state, ['awaiting_name','awaiting_email','awaiting_wa','awaiting_address','awaiting_postal','awaiting_confirmation'], true)) {
        ...
        try {
            ...
            if ($raw !== null && in_array(trim($raw), self::INTENTS, true)) {
                return trim($raw);
            }
        } catch (\Throwable $e) {
            error_log('[GeminiIntentDetector] detect: ' . $e->getMessage());
        }
    ...
    public function extractMultipleItems(string $message): array
    {
        try {
            ...
            if ($raw !== null) {
                ...
                if (is_array($decoded) && !empty($decoded)) {
                    return array_map(function ($item): array {
                        return [
                            'item_query' => (string)($item['item_query'] ?? ''),
            ...
            error_log('[GeminiIntentDetector] extractMultipleItems: ' . $e->getMessage());
        }

        if (method_exists($this->fallback, 'extractMultipleItems')) {
    ...
    private function looksLikeMenuExplanation(string $lower): bool
    {
        ...
        $hasMenuFmt  = str_contains($lower, 'rp') || str_contains($lower, '•') || str_contains($lower, 'paket ');
