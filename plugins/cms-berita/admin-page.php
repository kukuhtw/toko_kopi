...
echo cmsNewsRenderLayout($pageTitle, $content, $role);

function cmsNewsStartSession(): void
{
...
function cmsNewsRequireLogin(string $redirectTo = '/login.php'): void
{
...
function cmsNewsRenderLayout(string $title, string $content, string $role = 'super_admin'): string
{
    $user = cmsNewsUser();
    $initial = strtoupper(substr((string)($user['name'] ?? 'U'), 0, 1));
    $baseUrl = BASE_URL;
    $navItems = HookManager::applyFilters('dashboard.nav_items', cmsNewsDefaultNavItems($role), $role);
    $topbarActions = HookManager::applyFilters('dashboard.topbar_actions', '', (int)($user['branch_id'] ?? 0), $role);
    $headStyles = HookManager::applyFilters('dashboard.head_styles', '');
    $brandHtml = HookManager::applyFilters('dashboard.brand_html', '&#9749; Toko <span>Kopi</span>');
    $appName = HookManager::applyFilters('dashboard.app_name', APP_NAME);
...
<head>
  ...
  <title><?= htmlspecialchars($title) ?> - <?= htmlspecialchars((string)$appName) ?></title>
...
<div class="page-wrapper">
  ...
  <aside class="sidebar" id="dashboardSidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a href="<?= $baseUrl . $item['url'] ?>" class="nav-item <?= cmsNewsIsActive((string)$item['url']) ? 'active' : '' ?>">
    ...
    <header class="topbar">
      ...
      <div class="topbar-user">
        ...
        <div class="topbar-user-meta">
          <div style="font-weight:600"><?= htmlspecialchars((string)($user['name'] ?? '')) ?></div>
          <div style="font-size:.75rem;color:var(--text-light)"><?= htmlspecialchars(str_replace('_', ' ', (string)($user['role'] ?? ''))) ?></div>
        </div>
...
function cmsNewsIsActive(string $url): bool
{
    $current = parse_url((string)($_SERVER['REQUEST_URI'] ?? ''), PHP_URL_PATH);
    return $current === $url || str_starts_with((string)$current, $url);
}

function cmsNewsDefaultNavItems(string $role): array
{
    if ($role === 'super_admin') {
        return [
            'Overview' => [
