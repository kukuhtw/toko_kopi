...
final class ShopeeIntegrationPlugin implements PluginInterface
{
    private ShopeeIntegrationRepository $repo;
    private ShopeeIntegrationService $service;

    public function __construct()
    {
        $this->repo = new ShopeeIntegrationRepository();
        $this->service = new ShopeeIntegrationService($this->repo);
    }
    ...
    {
        if ($role === 'super_admin') {
            $items['Integrations'][] = ['url' => '/dashboard/super/shopee.php', 'icon' => 'SP', 'label' => 'Shopee'];
        }
        if ($role === 'branch_admin') {
            $items['Integrations'][] = ['url' => '/dashboard/branch/shopee.php', 'icon' => 'SP', 'label' => 'Shopee'];
        }
    ...
    {
        return ShopeeIntegrationRepository::PLUGIN_SLUG;
    }
    ...
    private function renderGlobalSettingsCard(): string
    {
        ...
        <div class="card" style="margin-top:16px">
          <div class="card-title">Shopee Global Runtime</div>
          <form method="POST">
            <?= Csrf::field() ?>
            ...
            <button type="submit" class="btn btn-primary">Simpan Runtime Shopee</button>
          </form>
        </div>
        <?php
        return ob_get_clean();
    ...
    private function renderBranchSettingsCard(int $branchId, bool $includeBranchField): string
    {
        ...
        $partnerId = $this->repo->getBranchSetting($branchId, 'partner_id');
        $partnerKey = $this->repo->getBranchSetting($branchId, 'partner_key');
        $shopId = $this->repo->getBranchSetting($branchId, 'shop_id');
        $accessToken = $this->repo->getBranchSetting($branchId, 'access_token');
        $refreshToken = $this->repo->getBranchSetting($branchId, 'refresh_token');
        ...
        $webhookUrl = BASE_URL . '/api/plugins/shopee/webhook.php?branch=' . $branchId;

        ob_start();
        ?>
        <div class="card" style="margin-top:16px">
          <div class="card-title">Shopee Integration</div>
          <div style="background:var(--bg-light,#faf9f7);border-radius:8px;padding:12px;margin-bottom:14px;font-size:.84rem;line-height:1.7">
            Scaffold awal untuk integrasi Shopee: OAuth, sinkron produk, stok, order, webhook, dan audit log.
            <br><br>
          ...
          <form method="POST">
            ...
            <div class="form-group">
              <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                ...
                <span>Aktifkan Shopee untuk cabang ini</span>
              </label>
            </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" name="partner_id" class="form-control" value="<?= htmlspecialchars($partnerId) ?>" placeholder="partner id dari Shopee">
              </div>
              <div class="form-group">
                <label class="form-label">Shop ID</label>
                <input type="text" name="shop_id" class="form-control" value="<?= htmlspecialchars($shopId) ?>" placeholder="shop id yang terhubung">
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="password" name="partner_key" class="form-control" value="<?= htmlspecialchars($partnerKey) ?>" placeholder="partner key / secret">
              </div>
              <div class="form-group">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" name="access_token" class="form-control" value="<?= htmlspecialchars($accessToken) ?>" placeholder="token OAuth aktif">
              </div>
              <div class="form-group">
                <label class="form-label">Refresh Token</label>
                <input type="text" name="refresh_token" class="form-control" value="<?= htmlspecialchars($refreshToken) ?>" placeholder="refresh token Shopee">
              </div>
            </div>
            ...
            <button type="submit" class="btn btn-primary">Simpan Konfigurasi Shopee</button>
          </form>
          ...
          <div style="margin-top:14px">
            ...
            <div style="overflow:auto;margin-top:8px">
              <table class="table" style="width:100%;font-size:.9rem">
                ...
                <?php endforeach; ?>
                <?php if ($recentLogs === []): ?>
                  <tr><td colspan="5" style="text-align:center;color:#666">Belum ada log sinkronisasi Shopee.</td></tr>
                <?php endif; ?>
                </tbody>
              </table>
            </div>
