...
class MetaWhatsAppPlugin implements PluginInterface
{
    private const PLUGIN_SLUG = 'meta-whatsapp';

    public function getName(): string
    {
        return 'Meta WhatsApp Gateway';
    }
    ...
    {
        $channels['whatsapp_meta'] = new MetaWhatsAppChannel();
        return $channels;
    }
    ...
    public function addSettingsSection(array $sections, int $branchId): array
    {
        ...
        $verifyToken = $this->getSetting($branchId, 'webhook_token');
        $phoneNumberId = $this->getSetting($branchId, 'phone_number_id');
        $legacyUsed = $this->isUsingLegacyFallback($branchId);
        $webhookUrl = BASE_URL . '/api/channel/webhook.php?channel=whatsapp_meta&branch=' . $branchId;

        ob_start();
        ?>
        <div class="card" style="margin-top:16px">
          <div class="card-title">Meta WhatsApp Gateway</div>
          ...
          <div style="background:#fff7e6;border:1px solid #f2d19a;color:#8a5a12;border-radius:10px;padding:12px 14px;margin-bottom:14px;font-size:.85rem;line-height:1.65">
            Konfigurasi ini masih membaca data Meta lama dari menu WhatsApp legacy.
          ...
          <div style="background:var(--bg-light,#faf9f7);border-radius:8px;padding:12px;margin-bottom:14px;font-size:.82rem;line-height:1.7">
            <strong>Webhook URL Meta</strong><br>
            <code style="word-break:break-all"><?= htmlspecialchars($webhookUrl) ?></code><br><br>
            Tempel URL ini sebagai webhook WhatsApp Cloud API untuk branch ini.
            Verifikasi GET dan inbound POST sekarang berjalan lewat sistem channel/plugin.
          </div>
          ...
          <form method="POST">
            ...
            <div class="form-row">
              <div class="form-group">
                <label class="form-label" for="meta_wa_number">Nomor WhatsApp Bot</label>
                <input type="text" id="meta_wa_number" name="wa_number" class="form-control"
                       value="<?= htmlspecialchars($waNumber) ?>"
                       placeholder="6281234567890">
              </div>
              <div class="form-group">
                <label class="form-label" for="meta_phone_number_id">Phone Number ID</label>
                <input type="text" id="meta_phone_number_id" name="phone_number_id" class="form-control"
                       value="<?= htmlspecialchars($phoneNumberId) ?>"
                       placeholder="Meta phone_number_id">
                <small style="color:var(--text-mid);font-size:.8rem">Wajib untuk outbound API Cloud WhatsApp.</small>
              </div>
            </div>

            <div class="form-group">
              <label class="form-label" for="meta_api_key">Access Token (Meta)</label>
              <input type="password" id="meta_api_key" name="api_key" class="form-control"
            ...
            <div class="form-row">
              <div class="form-group">
                <label class="form-label" for="meta_api_secret">App Secret</label>
                <input type="password" id="meta_api_secret" name="api_secret" class="form-control"
                       value="<?= htmlspecialchars($appSecret) ?>"
                       placeholder="Meta app secret">
              </div>
              <div class="form-group">
                <label class="form-label" for="meta_webhook_token">Webhook Verify Token</label>
                <input type="text" id="meta_webhook_token" name="webhook_token" class="form-control"
                       value="<?= htmlspecialchars($verifyToken) ?>"
                       placeholder="Token verifikasi webhook Meta">
              </div>
            </div>

            <button type="submit" class="btn btn-primary">Simpan Pengaturan Meta WhatsApp</button>
          </form>
        </div>
        <?php

        $sections[self::PLUGIN_SLUG] = ob_get_clean();
        return $sections;
    }
    ...
    private function getLegacySetting(int $branchId, string $key): string
    {
        ...
        $stmt->execute([$branchId, 'MetaCloudApiProvider']);
