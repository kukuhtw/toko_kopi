...
class OpenRouterProvider implements LlmProviderInterface
{
    ...
    public function completeJson(string $userMessage, string $systemPrompt, int $maxTokens = 400): ?string
    {
        $payload = [
            'model' => $this->model,
            'max_tokens' => $maxTokens,
    ...
    private function post(array $payload): string
    {
        if (!function_exists('curl_init')) {
            throw new \UnexpectedValueException('cURL extension is required for OpenRouterProvider.');
        }
